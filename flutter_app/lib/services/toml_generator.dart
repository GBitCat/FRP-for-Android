  ...
  if (c.isVisitor() && c.needsSecretKey()) {
    ...
    if (c.protocol == 'xtcp') {
      ...
      if (c.useFallback && c.fallbackTo.isNotEmpty) {
        b.writeln('fallbackTo = "${c.fallbackTo}"');
...
String globalBlock(ServerConfig server) {
  final b = StringBuffer();
  b.writeln('serverAddr = "${server.serverAddr}"');
  b.writeln('serverPort = ${server.serverPort}');
  if (server.token.isNotEmpty) b.writeln('auth.token = "${server.token}"');
  b.writeln('natHoleStunServer = "stun.easyvoip.com:3478"');
  b.writeln();
  b.writeln('transport.protocol = "${server.protocol}"');
